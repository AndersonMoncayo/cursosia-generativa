...
export function CourseFilters({ currentLevel }: { currentLevel: string | null }) {
            ...
            return (
              <label
                ...
                className={`flex items-center gap-2 cursor-pointer py-1 px-2 border-2 transition-colors ${
                  isActive ? 'border-primary bg-slate-100' : 'border-transparent hover:border-slate-300'
                }`}
                onClick={(e) => {
                  e.preventDefault()
              ...
              >
                ...
                />
                <span className={`font-mono text-[13px] uppercase ${isActive ? 'font-black text-black' : 'font-bold text-slate-600'}`}>
                  {nivel.label}
                </span>
              </label>
            )

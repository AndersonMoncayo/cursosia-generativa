...
export async function updateModuleInline(id: string, data: any) {
  ...
  const { error } = await supabase.from('modules').update(finalData).eq('id', id)
  if (error) return { success: false, error: error.message }
  return { success: true }
}

export async function toggleModulePublishInline(id: string, is_published: boolean) {
...
export async function createLessonInline(course_id: string, module_id: string) {
  ...
  const { data: moduleData } = await supabase.from('modules').select('course_id').eq('id', module_id).single()
...
export async function updateLessonInline(id: string, data: any) {
  ...
  const finalData = { ...parsed.data, updated_at: new Date().toISOString() }
  const { error } = await supabase.from('lessons').update(finalData).eq('id', id)
  if (error) return { success: false, error: error.message }
  return { success: true }
}

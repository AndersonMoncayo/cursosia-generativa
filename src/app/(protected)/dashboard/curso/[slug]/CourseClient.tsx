...
export const CourseClient: React.FC<CourseClientProps> = ({ courseId, courseTitle, courseSlug, modules }) => {
  ...
  const handleCheckboxChange = (index: number) => {
    ...
    localStorage.setItem(`progress_${courseSlug}_${index}`, String(newState[index]))
  }
      ...
      <div className="bg-black border-4 border-slate-700 p-8 retro-shadow">
        ...
        <div className="space-y-4 mb-8">
          {modules.map((mod, index) => (
            <div key={index} className="bg-background-dark border-4 border-slate-700 p-6 flex flex-col md:flex-row md:justify-between md:items-center hover:border-primary transition-colors cursor-pointer" onClick={() => handleCheckboxChange(index)}>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <Link href="/dashboard" className="flex-1 bg-white text-black font-black uppercase tracking-widest py-4 border-4 border-black text-center text-lg retro-btn hover:bg-slate-200 transition-colors">
            VOLVER AL PANEL
          </Link>
        </div>
      </div>

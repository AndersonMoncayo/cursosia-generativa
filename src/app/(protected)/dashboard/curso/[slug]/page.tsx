...
export default async function DashboardCourseDetail({
  ...
  // Verify enrollment
  const isEnrolled = course.enrollments.some((enr: any) => enr.user_id === user.id)
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 lg:py-20">
        <div className="mb-12 border-b-8 border-primary pb-8">
           ...
           <div className="inline-flex items-center gap-3 bg-black border-4 border-primary px-4 py-2 retro-shadow text-primary font-mono text-sm uppercase">
             ...
             <span className="text-white font-black">{course.slug.toUpperCase()}</span>
           </div>
        </div>

        <CourseClient courseId={course.id} courseTitle={course.title} courseSlug={course.slug} modules={modules} />

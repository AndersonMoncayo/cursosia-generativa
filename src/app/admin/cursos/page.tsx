...
export default function AdminCursos() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (res.ok) {
        ...
        toast.error(data.error || 'Error fetch courses')
      }
    } catch (e: any) {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!confirm(`¿Eliminar ESTRICTAMENTE el curso "${title}"? Esta acción es irreversible.`)) return
    
    try {
      const res = await fetch(`/api/admin/courses/${id}`, { method: 'DELETE' })
      if (res.ok) {
        toast.success('Curso eliminado del sistema')
        setCourses(courses.filter(c => c.id !== id))
      } else {
        const data = await res.json()
        toast.error(data.error || 'Error eliminando curso')
      }
    } catch (e: any) {
      toast.error(e.message)
    }
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="bg-black border-4 border-slate-700 retro-shadow p-8">
        {loading ? (
          <div className="text-center font-bold text-slate-400 p-8 uppercase tracking-widest">
            CARGANDO DATOS...
          </div>
        ) : courses.length === 0 ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b-4 border-slate-700">
                  ...
                  <th className="py-4 px-4 font-black uppercase text-xs text-primary tracking-widest bg-slate-900">Instructor</th>
                  <th className="py-4 px-4 font-black uppercase text-xs text-primary tracking-widest bg-slate-900 text-center">Inscritos</th>
              ...
              <tbody>
                {courses.map((c: any) => (
                  <tr key={c.id} className="border-b-2 border-slate-800 hover:bg-slate-900 transition-colors">
                    ...
                    <td className="py-4 px-4 text-slate-400 font-bold uppercase text-xs">{c.instructor_nombre || 'N/A'}</td>
                    <td className="py-4 px-4 text-white font-black text-center">{c.enrollments_count}</td>
                    ...
                    <td className="py-4 px-4">
                      <div className="flex justify-center gap-2">
                         ...
                         <button onClick={() => handleDelete(c.id, c.title)} className="bg-black text-red-500 p-2 border-2 border-slate-600 hover:border-red-500 hover:bg-red-500 hover:text-white transition-colors" title="Eliminar">
